 ...
 */
export function conversationSessionId(body: unknown): string | undefined {
...
export function withOpencodeSessionHeader(
  inner: FetchLike,
  fallbackId: string = `dsh-${randomUUID()}`,
  logger?: PluginLogger,
): FetchLike {
 ...
 */
export function installOpencodeSessionHeader(logger?: PluginLogger): () => void {
  ...
  const patched = withOpencodeSessionHeader(original as FetchLike, `dsh-${randomUUID()}`, logger)
