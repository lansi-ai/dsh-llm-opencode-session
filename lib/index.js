 ...
 */
export function sessionLabel(source, style = 'cjk') {
 ...
 */
export function toHeaderValue(label) {
 ...
 */
export function conversationSessionId(body, style = 'cjk') {
 ...
 * @returns 包装后的 fetch。
 */
export function withOpencodeSessionHeader(inner, fallbackId, logger, style = 'cjk') {
 ...
 */
export function installOpencodeSessionHeader(logger, config) {
    if (installed !== null)
        return installed;
    ...
    const style = config?.labelStyle === 'ascii' ? 'ascii' : 'cjk';
    const patched = withOpencodeSessionHeader(original, sessionLabel(randomUUID(), style), logger, style);
    globalThis.fetch = patched;
    emit(logger, 'info', `${TAG} opencode 会话头注入已安装（仅 opencode.ai 域；标签风格 ${style}；已带该头的请求不覆盖）`);
    const restore = () => {
...
export function apply(ctx, config) {
    try {
        ctx.effect(() => installOpencodeSessionHeader(ctx.logger, config), 'llm-opencode-session: fetch header patch');
    }
    catch (error) {
